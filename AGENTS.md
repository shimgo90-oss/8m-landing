...
- **PostHog** = the home for behavioural data (autocaptured pageviews/clicks, session replay, heatmaps, funnels). Loaded via the official snippet in [app/layout.tsx](app/layout.tsx).
- **The landing shares ONE PostHog project with the product app** (the free plan allows a single project). Every event from this site carries a **`site: "landing"`** super property — filter on it for landing-only numbers; exclude it for product-app-only numbers. Any new query or report must respect that filter, or it will silently mix the two sites.
